...
export function ContentDashboard({ data }) {
  const { cadence, formats, timeline, topics, weeklyCalendar } = data;
  ...
  const clusters = useMemo(() => ["Semua", ...new Set(topics.map((topic) => topic.cluster))], [topics]);
  const pillars = useMemo(() => ["Semua", ...new Set(topics.map((topic) => topic.pillar))], [topics]);
  ...
  }, [cluster, pillar, query, topics]);

  const selectedTopic = topics.find((topic) => topic.id === selectedTopicId) ?? filteredTopics[0] ?? topics[0] ?? null;
          ...
          <header className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-3xl font-semibold tracking-normal">
                {activeView === "timeline" ? "Timeline Publikasi" : "Bank Topik Konten"}
              </h2>
            </div>
          </header>
    ...
    <div className="grid gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          ...
          {selectedTopic.folders.length > 0 && (
